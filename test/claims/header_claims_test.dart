...
void main() {
  ...
  late HeaderClaims headerClaims;
  setUp(() {
    headerClaims = HeaderClaims(JwtBuilder.fromToken(token));
  });

  group('Payload Keycloak part', () {
    test('typ', () {
      expect(headerClaims.typ, 'JWT');
    });

    test('alg', () {
      expect(headerClaims.alg, 'RS256');
    });

    test('kid', () {
      expect(headerClaims.kid, '5uuFbCbGjydDi87I-pqn7n-DY3ijyD_KyXyn_GfRhyg');
    });
  });
}
