...
void main() {
  ...
  late KeycloakClaims keycloakClaims;
  setUp(() {
    keycloakClaims = KeycloakClaims(JwtBuilder.fromToken(token));
  });

  group('Payload Keycloak part', () {
    test('rolesRealmAccess', () {
      expect(keycloakClaims.rolesRealmAccess.length, 3);
      expect(keycloakClaims.rolesRealmAccess.toString(),
          '[default-roles-terra-media, offline_access, uma_authorization]');
    });

    test('rolesAccount', () {
      expect(keycloakClaims.rolesAccount.length, 3);
      expect(keycloakClaims.rolesAccount.toString(),
          '[manage-account, manage-account-links, view-profile]');
    });

    test('typ', () {
      expect(keycloakClaims.typ, 'Bearer');
    });
  });
}
